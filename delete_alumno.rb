...
require 'mysql2'
include Fox
class WindowdeleteAlu <FXMainWindow
  ...
  def initialize(app)
    ...
    full=super(app,"Elimina alumnos", :icon => full_icon,:width=>600, :height=>220)
    ...
    lbl=FXLabel.new( self,"ELIMINAR ALUMNO", :opts=>LAYOUT_EXPLICIT, :width=>560, :height=>50, :x=>20, :y=>10)
    lbl.font = helvetica2
  ...
  def eliminar_docente
    ...
    if answer == MBOX_CLICKED_YES
      consu="DELETE FROM `alumnos` WHERE `alumnos`.`ID_Alumno` = #{id}"
      resul=@conn.query(consu)
      FXMessageBox.information(self,MBOX_OK,"Atención","Alumno Eliminado")
      clean_inputs
    end
  end
